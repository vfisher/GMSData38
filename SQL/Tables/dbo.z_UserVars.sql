CREATE TABLE [dbo].[z_UserVars] (
  ...
  [ObjectDef] [text] NULL,
  CONSTRAINT [pk_z_UserVars] PRIMARY KEY CLUSTERED ([UserCode], [VarName])
)
ON [PRIMARY]
...
CREATE INDEX [UserCode]
  ON [dbo].[z_UserVars] ([UserCode])
  ON [PRIMARY]
GO

CREATE INDEX [VarName]
  ON [dbo].[z_UserVars] ([VarName])
  ON [PRIMARY]
GO

CREATE INDEX [VarPosID]
  ON [dbo].[z_UserVars] ([UserCode], [VarPosID])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_z_UserVars] ON [z_UserVars]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_z_UserVars', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_z_UserVars] ON [z_UserVars]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_z_UserVars', N'Last', N'UPDATE'
GO

ALTER TABLE [dbo].[z_UserVars]
  ADD CONSTRAINT [FK_z_UserVars_r_Users] FOREIGN KEY ([UserCode]) REFERENCES [dbo].[r_Users] ([UserID]) ON DELETE CASCADE ON UPDATE CASCADE
GO