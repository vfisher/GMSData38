CREATE TABLE [dbo].[r_EmpInc] (
  ...
  CONSTRAINT [_pk_r_EmpInc] PRIMARY KEY CLUSTERED ([SrcPosID], [EmpID])
)
ON [PRIMARY]
GO

CREATE INDEX [BDate]
  ON [dbo].[r_EmpInc] ([BDate])
  ON [PRIMARY]
GO

CREATE INDEX [EDate]
  ON [dbo].[r_EmpInc] ([EDate])
  ON [PRIMARY]
GO

CREATE UNIQUE INDEX [NoDuplicate]
  ON [dbo].[r_EmpInc] ([OurID], [Priority], [EmpID])
  ON [PRIMARY]
GO

CREATE INDEX [PayTypeID]
  ON [dbo].[r_EmpInc] ([PayTypeID])
  ON [PRIMARY]
GO

CREATE INDEX [r_EmpMOr_EmpInc]
  ON [dbo].[r_EmpInc] ([OurID], [EmpID])
  ON [PRIMARY]
GO

CREATE INDEX [SrcPosID]
  ON [dbo].[r_EmpInc] ([SrcPosID])
  ON [PRIMARY]
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.r_EmpInc.OurID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.r_EmpInc.SrcPosID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.r_EmpInc.EmpID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.r_EmpInc.PayTypeID'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_r_EmpInc] ON [r_EmpInc]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_r_EmpInc', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_r_EmpInc] ON [r_EmpInc]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_r_EmpInc', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_r_EmpInc] ON [r_EmpInc]
FOR DELETE AS
...
EXEC sp_settriggerorder N'dbo.TRel3_Del_r_EmpInc', N'Last', N'DELETE'
GO