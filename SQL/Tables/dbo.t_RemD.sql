CREATE TABLE [dbo].[t_RemD] (
  ...
  CONSTRAINT [_pk_t_RemD] PRIMARY KEY CLUSTERED ([OurID], [StockID], [SecID], [ProdID], [PPID])
)
ON [PRIMARY]
GO
...
CREATE INDEX [StockID]
  ON [dbo].[t_RemD] ([StockID])
  ON [PRIMARY]
GO

CREATE INDEX [t_PInP_Tt_RemD]
  ON [dbo].[t_RemD] ([ProdID], [PPID])
  ON [PRIMARY]
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.t_RemD.OurID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.t_RemD.StockID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.t_RemD.SecID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.t_RemD.ProdID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.t_RemD.PPID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.t_RemD.Qty'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_t_RemD] ON [t_RemD]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_t_RemD', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_t_RemD] ON [t_RemD]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_t_RemD', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_t_RemD] ON [t_RemD]FOR DELETE AS/* t_RemD - Остатки товара на дату (Таблица) - DELETE TRIGGER */BEGIN  SET NOCOUNT ON/* Удаление регистрации создания записи */  DELETE z_LogCreate FROM z_LogCreate m, deleted i  WHERE m.TableCode = 11904001 AND m.PKValue =     '[' + cast(i.OurID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.StockID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.SecID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.ProdID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.PPID as varchar(200)) + ']'/* Удаление регистрации изменения записи */  DELETE z_LogUpdate FROM z_LogUpdate m, deleted i  WHERE m.TableCode = 11904001 AND m.PKValue =     '[' + cast(i.OurID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.StockID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.SecID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.ProdID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.PPID as varchar(200)) + ']'/* Регистрация удаления записи */  INSERT INTO z_LogDelete (TableCode, ChID, PKValue, UserCode)  SELECT 11904001, 0,     '[' + cast(d.OurID as varchar(200)) + ']' + ' \ ' +     '[' + cast(d.StockID as varchar(200)) + ']' + ' \ ' +     '[' + cast(d.SecID as varchar(200)) + ']' + ' \ ' +     '[' + cast(d.ProdID as varchar(200)) + ']' + ' \ ' +     '[' + cast(d.PPID as varchar(200)) + ']'          , dbo.zf_GetUserCode() FROM deleted dEND
GO

EXEC sp_settriggerorder N'dbo.TRel3_Del_t_RemD', N'Last', N'DELETE'
GO