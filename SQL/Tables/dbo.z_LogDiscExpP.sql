CREATE TABLE [dbo].[z_LogDiscExpP] (
  ...
  CONSTRAINT [pk_z_LogDiscExpP] PRIMARY KEY CLUSTERED ([DBiID], [LogID])
)
ON [PRIMARY]
GO

CREATE INDEX [DocCode_ChID_SrcPosID]
  ON [dbo].[z_LogDiscExpP] ([DocCode], [ChID], [SrcPosID])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_z_LogDiscExpP] ON [z_LogDiscExpP]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_z_LogDiscExpP', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_z_LogDiscExpP] ON [z_LogDiscExpP]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_z_LogDiscExpP', N'Last', N'UPDATE'
GO

ALTER TABLE [dbo].[z_LogDiscExpP]
  ADD CONSTRAINT [FK_z_LogDiscExpP_r_DCards] FOREIGN KEY ([DCardChID]) REFERENCES [dbo].[r_DCards] ([ChID]) ON UPDATE CASCADE
GO