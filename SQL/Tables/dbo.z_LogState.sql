CREATE TABLE [dbo].[z_LogState] (
  ...
  CONSTRAINT [pk_z_LogState] PRIMARY KEY CLUSTERED ([LogID])
)
ON [PRIMARY]
GO

CREATE INDEX [UserCode]
  ON [dbo].[z_LogState] ([UserCode])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_z_LogState] ON [z_LogState]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_z_LogState', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_z_LogState] ON [z_LogState]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_z_LogState', N'Last', N'UPDATE'
GO

ALTER TABLE [dbo].[z_LogState]
  ADD CONSTRAINT [FK_z_LogState_r_Users] FOREIGN KEY ([UserCode]) REFERENCES [dbo].[r_Users] ([UserID]) ON UPDATE CASCADE
GO

ALTER TABLE [dbo].[z_LogState]
  ADD CONSTRAINT [FK_z_LogState_z_Docs] FOREIGN KEY ([DocCode]) REFERENCES [dbo].[z_Docs] ([DocCode]) ON DELETE CASCADE ON UPDATE CASCADE
GO