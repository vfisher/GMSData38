CREATE TABLE [dbo].[r_ProdAC] (
  ...
  CONSTRAINT [_pk_r_ProdAC] PRIMARY KEY CLUSTERED ([ProdID], [PLID], [ChPLID])
)
ON [PRIMARY]
GO

CREATE INDEX [ChPLID]
  ON [dbo].[r_ProdAC] ([ChPLID])
  ON [PRIMARY]
GO

CREATE INDEX [ExpE]
  ON [dbo].[r_ProdAC] ([ExpE])
  ON [PRIMARY]
GO

CREATE INDEX [ExpR]
  ON [dbo].[r_ProdAC] ([ExpR])
  ON [PRIMARY]
GO

CREATE INDEX [PLID]
  ON [dbo].[r_ProdAC] ([PLID])
  ON [PRIMARY]
GO

CREATE INDEX [r_ProdMPr_ProdAC]
  ON [dbo].[r_ProdAC] ([ProdID], [PLID])
  ON [PRIMARY]
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.r_ProdAC.ProdID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.r_ProdAC.PLID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.r_ProdAC.ChPLID'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_r_ProdAC] ON [r_ProdAC]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_r_ProdAC', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_r_ProdAC] ON [r_ProdAC]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_r_ProdAC', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_r_ProdAC] ON [r_ProdAC]
FOR DELETE AS
...
EXEC sp_settriggerorder N'dbo.TRel3_Del_r_ProdAC', N'Last', N'DELETE'
GO