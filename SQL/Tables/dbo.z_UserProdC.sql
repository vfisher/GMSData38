CREATE TABLE [dbo].[z_UserProdC] (
  ...
  CONSTRAINT [pk_z_UserProdC] PRIMARY KEY CLUSTERED ([UserID], [PCatID])
)
ON [PRIMARY]
GO

CREATE INDEX [AccDelete]
  ON [dbo].[z_UserProdC] ([AccDelete])
  ON [PRIMARY]
GO

CREATE INDEX [AccInsert]
  ON [dbo].[z_UserProdC] ([AccInsert])
  ON [PRIMARY]
GO

CREATE INDEX [AccRead]
  ON [dbo].[z_UserProdC] ([AccRead])
  ON [PRIMARY]
GO

CREATE INDEX [AccUpdate]
  ON [dbo].[z_UserProdC] ([AccUpdate])
  ON [PRIMARY]
GO

CREATE INDEX [PCatID]
  ON [dbo].[z_UserProdC] ([PCatID])
  ON [PRIMARY]
GO

CREATE INDEX [UserID]
  ON [dbo].[z_UserProdC] ([UserID])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_z_UserProdC] ON [z_UserProdC]
FOR UPDATE AS
...
/* Доступные значения - Справочник товаров: 1 группа ^ Справочник товаров: категории - Проверка в PARENT */
  IF UPDATE(PCatID)
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_z_UserProdC', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_z_UserProdC] ON [z_UserProdC]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_z_UserProdC', N'Last', N'INSERT'
GO
