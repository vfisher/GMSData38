CREATE TABLE [dbo].[z_DocLinkTypes] (
  ...
  [SignType] [int] NULL,
  CONSTRAINT [pk_z_DocLinkTypes] PRIMARY KEY CLUSTERED ([DocLinkTypeID])
)
ON [PRIMARY]
GO

CREATE UNIQUE INDEX [DocLinkTypeName]
  ON [dbo].[z_DocLinkTypes] ([DocLinkTypeName])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_z_DocLinkTypes] ON [z_DocLinkTypes]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_z_DocLinkTypes', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_z_DocLinkTypes] ON [z_DocLinkTypes]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_z_DocLinkTypes', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_z_DocLinkTypes] ON [z_DocLinkTypes]FOR DELETE AS/* z_DocLinkTypes - Документы - Взаимосвязи: Типы - DELETE TRIGGER */BEGIN  SET NOCOUNT ON/* Удаление регистрации создания записи */  DELETE z_LogCreate FROM z_LogCreate m, deleted i  WHERE m.TableCode = 1001071 AND m.PKValue =     '[' + cast(i.DocLinkTypeID as varchar(200)) + ']'/* Удаление регистрации изменения записи */  DELETE z_LogUpdate FROM z_LogUpdate m, deleted i  WHERE m.TableCode = 1001071 AND m.PKValue =     '[' + cast(i.DocLinkTypeID as varchar(200)) + ']'/* Регистрация удаления записи */  INSERT INTO z_LogDelete (TableCode, ChID, PKValue, UserCode)  SELECT 1001071, 0,     '[' + cast(d.DocLinkTypeID as varchar(200)) + ']'          , dbo.zf_GetUserCode() FROM deleted dEND
GO

EXEC sp_settriggerorder N'dbo.TRel3_Del_z_DocLinkTypes', N'Last', N'DELETE'
GO