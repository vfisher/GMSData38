CREATE TABLE [dbo].[r_DiscDC] (
  ...
  [ForRec] [bit] NOT NULL,
  [ForExp] [bit] NOT NULL,
  CONSTRAINT [pk_r_DiscDC] PRIMARY KEY CLUSTERED ([DiscCode], [DCTypeCode])
)
ON [PRIMARY]
GO
...
GO
CREATE TRIGGER [dbo].[TRel1_Ins_r_DiscDC] ON [r_DiscDC]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_r_DiscDC', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_r_DiscDC] ON [r_DiscDC]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_r_DiscDC', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_r_DiscDC] ON [r_DiscDC]
FOR DELETE AS
...
EXEC sp_settriggerorder N'dbo.TRel3_Del_r_DiscDC', N'Last', N'DELETE'
GO

ALTER TABLE [dbo].[r_DiscDC]
  ADD CONSTRAINT [FK_r_DiscDC_r_DCTypes] FOREIGN KEY ([DCTypeCode]) REFERENCES [dbo].[r_DCTypes] ([DCTypeCode]) ON DELETE CASCADE ON UPDATE CASCADE
GO

ALTER TABLE [dbo].[r_DiscDC]
  ADD CONSTRAINT [FK_r_DiscDC_r_Discs] FOREIGN KEY ([DiscCode]) REFERENCES [dbo].[r_Discs] ([DiscCode]) ON DELETE CASCADE ON UPDATE CASCADE
GO