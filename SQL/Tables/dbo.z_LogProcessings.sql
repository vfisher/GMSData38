CREATE TABLE [dbo].[z_LogProcessings] (
  ...
  CONSTRAINT [pk_z_LogProcessings] PRIMARY KEY CLUSTERED ([DocCode], [ChID])
)
ON [PRIMARY]
GO

CREATE INDEX [Status]
  ON [dbo].[z_LogProcessings] ([Status])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_z_LogProcessings] ON [z_LogProcessings]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_z_LogProcessings', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_z_LogProcessings] ON [z_LogProcessings]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_z_LogProcessings', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_z_LogProcessings] ON [z_LogProcessings]FOR DELETE AS/* z_LogProcessings - Регистрация действий – Процессинг - DELETE TRIGGER */BEGIN  SET NOCOUNT ON/* Удаление регистрации печати */  DELETE z_LogPrint FROM z_LogPrint m, deleted i  WHERE m.DocCode = 1001 AND m.ChID = i.ChIDEND
GO

EXEC sp_settriggerorder N'dbo.TRel3_Del_z_LogProcessings', N'Last', N'DELETE'
GO