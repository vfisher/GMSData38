CREATE TABLE [dbo].[r_TaxRates] (
  ...
  CONSTRAINT [pk_r_TaxRates] PRIMARY KEY CLUSTERED ([TaxTypeID], [ChDate])
)
ON [PRIMARY]
GO
...
GO
CREATE TRIGGER [dbo].[TRel1_Ins_r_TaxRates] ON [r_TaxRates]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_r_TaxRates', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_r_TaxRates] ON [r_TaxRates]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_r_TaxRates', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_r_TaxRates] ON [r_TaxRates]FOR DELETE AS/* r_TaxRates - Справочник НДС: значения - DELETE TRIGGER */BEGIN  SET NOCOUNT ON/* Удаление регистрации создания записи */  DELETE z_LogCreate FROM z_LogCreate m, deleted i  WHERE m.TableCode = 10810002 AND m.PKValue =     '[' + cast(i.TaxTypeID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.ChDate as varchar(200)) + ']'/* Удаление регистрации изменения записи */  DELETE z_LogUpdate FROM z_LogUpdate m, deleted i  WHERE m.TableCode = 10810002 AND m.PKValue =     '[' + cast(i.TaxTypeID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.ChDate as varchar(200)) + ']'/* Регистрация удаления записи */  INSERT INTO z_LogDelete (TableCode, ChID, PKValue, UserCode)  SELECT 10810002, 0,     '[' + cast(d.TaxTypeID as varchar(200)) + ']' + ' \ ' +     '[' + cast(d.ChDate as varchar(200)) + ']'          , dbo.zf_GetUserCode() FROM deleted dEND
GO

EXEC sp_settriggerorder N'dbo.TRel3_Del_r_TaxRates', N'Last', N'DELETE'
GO