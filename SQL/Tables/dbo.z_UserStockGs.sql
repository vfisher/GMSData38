CREATE TABLE [dbo].[z_UserStockGs] (
  ...
  CONSTRAINT [pk_z_UserStockGs] PRIMARY KEY CLUSTERED ([UserID], [StockGID])
)
ON [PRIMARY]
GO

CREATE INDEX [AccDelete]
  ON [dbo].[z_UserStockGs] ([AccDelete])
  ON [PRIMARY]
GO

CREATE INDEX [AccInsert]
  ON [dbo].[z_UserStockGs] ([AccInsert])
  ON [PRIMARY]
GO

CREATE INDEX [AccRead]
  ON [dbo].[z_UserStockGs] ([AccRead])
  ON [PRIMARY]
GO

CREATE INDEX [AccUpdate]
  ON [dbo].[z_UserStockGs] ([AccUpdate])
  ON [PRIMARY]
GO

CREATE INDEX [StockGID]
  ON [dbo].[z_UserStockGs] ([StockGID])
  ON [PRIMARY]
GO

CREATE INDEX [UserID]
  ON [dbo].[z_UserStockGs] ([UserID])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_z_UserStockGs] ON [z_UserStockGs]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_z_UserStockGs', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_z_UserStockGs] ON [z_UserStockGs]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_z_UserStockGs', N'Last', N'UPDATE'
GO