CREATE TABLE [dbo].[r_PersonDC] (
  ...
  CONSTRAINT [pk_r_PersonDC] PRIMARY KEY CLUSTERED ([PersonID], [DCardChID])
)
ON [PRIMARY]
GO

CREATE UNIQUE INDEX [DCardChID]
  ON [dbo].[r_PersonDC] ([DCardChID])
  ON [PRIMARY]
GO

CREATE INDEX [PersonID]
  ON [dbo].[r_PersonDC] ([PersonID])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_r_PersonDC] ON [r_PersonDC]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_r_PersonDC', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_r_PersonDC] ON [r_PersonDC]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_r_PersonDC', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_r_PersonDC] ON [r_PersonDC]
FOR DELETE AS
...
EXEC sp_settriggerorder N'dbo.TRel3_Del_r_PersonDC', N'Last', N'DELETE'
GO

ALTER TABLE [dbo].[r_PersonDC]
  ADD CONSTRAINT [FK_r_PersonDC_r_DCards] FOREIGN KEY ([DCardChID]) REFERENCES [dbo].[r_DCards] ([ChID]) ON UPDATE CASCADE
GO