CREATE TABLE [dbo].[t_RestShift] (
  ...
  CONSTRAINT [pk_t_RestShift] PRIMARY KEY CLUSTERED ([ChID])
)
ON [PRIMARY]
GO

CREATE INDEX [DocDate]
  ON [dbo].[t_RestShift] ([DocDate])
  ON [PRIMARY]
GO

CREATE INDEX [DocID]
  ON [dbo].[t_RestShift] ([DocID])
  ON [PRIMARY]
GO

CREATE INDEX [EmpID]
  ON [dbo].[t_RestShift] ([EmpID])
  ON [PRIMARY]
GO

CREATE INDEX [OurID]
  ON [dbo].[t_RestShift] ([OurID])
  ON [PRIMARY]
GO

CREATE INDEX [StockID]
  ON [dbo].[t_RestShift] ([StockID])
  ON [PRIMARY]
GO

CREATE INDEX [UniqueIndex]
  ON [dbo].[t_RestShift] ([OurID], [DocID])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_t_RestShift] ON [t_RestShift]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_t_RestShift', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_t_RestShift] ON [t_RestShift]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_t_RestShift', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_t_RestShift] ON [t_RestShift]
FOR DELETE AS
...
EXEC sp_settriggerorder N'dbo.TRel3_Del_t_RestShift', N'Last', N'DELETE'
GO