CREATE TABLE [dbo].[z_ReplicaFilters] (
  ...
  CONSTRAINT [pk_z_ReplicaFilters] PRIMARY KEY NONCLUSTERED ([ReplicaPubCode], [TableCode], [PFieldNames], [CTableCode])
)
ON [PRIMARY]
GO

CREATE INDEX [ReplicaPubCode]
  ON [dbo].[z_ReplicaFilters] ([ReplicaPubCode])
  ON [PRIMARY]
GO

CREATE INDEX [TableCode]
  ON [dbo].[z_ReplicaFilters] ([TableCode])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_z_ReplicaFilters] ON [z_ReplicaFilters]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_z_ReplicaFilters', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_z_ReplicaFilters] ON [z_ReplicaFilters]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_z_ReplicaFilters', N'Last', N'UPDATE'
GO

ALTER TABLE [dbo].[z_ReplicaFilters]
  ADD CONSTRAINT [FK_z_ReplicaFilters_z_ReplicaTables] FOREIGN KEY ([ReplicaPubCode], [TableCode]) REFERENCES [dbo].[z_ReplicaTables] ([ReplicaPubCode], [TableCode]) ON DELETE CASCADE ON UPDATE CASCADE
GO