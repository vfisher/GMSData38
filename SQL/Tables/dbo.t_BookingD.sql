CREATE TABLE [dbo].[t_BookingD] (
  ...
  [ForRet] [bit] NOT NULL,
  CONSTRAINT [pk_t_BookingD] PRIMARY KEY CLUSTERED ([ChID], [SrcPosID])
)
ON [PRIMARY]
GO

CREATE INDEX [ChID]
  ON [dbo].[t_BookingD] ([ChID])
  ON [PRIMARY]
GO

CREATE UNIQUE INDEX [ChID_DetSrcPosID]
  ON [dbo].[t_BookingD] ([ChID], [DetSrcPosID])
  ON [PRIMARY]
GO

CREATE INDEX [ResourceID]
  ON [dbo].[t_BookingD] ([ResourceID])
  ON [PRIMARY]
GO

CREATE INDEX [SrcPosID]
  ON [dbo].[t_BookingD] ([SrcPosID])
  ON [PRIMARY]
GO

CREATE INDEX [SrvcID]
  ON [dbo].[t_BookingD] ([SrvcID])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_t_BookingD] ON [t_BookingD]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_t_BookingD', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_t_BookingD] ON [t_BookingD]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_t_BookingD', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_t_BookingD] ON [t_BookingD]
FOR DELETE AS
...
EXEC sp_settriggerorder N'dbo.TRel3_Del_t_BookingD', N'Last', N'DELETE'
GO