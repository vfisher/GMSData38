CREATE TABLE [dbo].[z_UserStocks] (
  ...
  CONSTRAINT [pk_z_UserStocks] PRIMARY KEY CLUSTERED ([UserID], [StockID])
)
ON [PRIMARY]
GO

CREATE INDEX [AccDelete]
  ON [dbo].[z_UserStocks] ([AccDelete])
  ON [PRIMARY]
GO

CREATE INDEX [AccInsert]
  ON [dbo].[z_UserStocks] ([AccInsert])
  ON [PRIMARY]
GO

CREATE INDEX [AccRead]
  ON [dbo].[z_UserStocks] ([AccRead])
  ON [PRIMARY]
GO

CREATE INDEX [AccUpdate]
  ON [dbo].[z_UserStocks] ([AccUpdate])
  ON [PRIMARY]
GO

CREATE INDEX [StockID]
  ON [dbo].[z_UserStocks] ([StockID])
  ON [PRIMARY]
GO

CREATE INDEX [UserID]
  ON [dbo].[z_UserStocks] ([UserID])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_z_UserStocks] ON [z_UserStocks]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_z_UserStocks', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_z_UserStocks] ON [z_UserStocks]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_z_UserStocks', N'Last', N'UPDATE'
GO