CREATE TABLE [dbo].[r_StateRules] (
  ...
  CONSTRAINT [pk_r_StateRules] PRIMARY KEY CLUSTERED ([StateRuleCode])
)
ON [PRIMARY]
GO

CREATE INDEX [StateCodeFrom]
  ON [dbo].[r_StateRules] ([StateCodeFrom])
  ON [PRIMARY]
GO

CREATE INDEX [StateCodeTo]
  ON [dbo].[r_StateRules] ([StateCodeTo])
  ON [PRIMARY]
GO

CREATE UNIQUE INDEX [UniqueIndex]
  ON [dbo].[r_StateRules] ([StateCodeFrom], [StateCodeTo])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_r_StateRules] ON [r_StateRules]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_r_StateRules', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_r_StateRules] ON [r_StateRules]
FOR UPDATE AS
