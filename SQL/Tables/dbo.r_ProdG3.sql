...
FOR DELETE AS
/* r_ProdG3 - Справочник товаров: 3 группа - DELETE TRIGGER */
BEGIN
  SET NOCOUNT ON
...
FOR UPDATE AS
/* r_ProdG3 - Справочник товаров: 3 группа - UPDATE TRIGGER */
BEGIN
  DECLARE @RCount Int
...
FOR INSERT AS
/* r_ProdG3 - Справочник товаров: 3 группа - INSERT TRIGGER */
BEGIN
  DECLARE @RCount Int
  SELECT @RCount = @@RowCount
  IF @RCount = 0 RETURN
