...
CREATE UNIQUE INDEX [ChID]
  ON [dbo].[r_WPRoles] ([ChID])
  ON [PRIMARY]
GO

CREATE INDEX [MenuID]
  ON [dbo].[r_WPRoles] ([MenuID])
  ON [PRIMARY]
GO

CREATE INDEX [ProcessingID]
  ON [dbo].[r_WPRoles] ([ProcessingID])
  ON [PRIMARY]
GO

CREATE UNIQUE INDEX [WPRoleName]
  ON [dbo].[r_WPRoles] ([WPRoleName])
  ON [PRIMARY]
GO
...
SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_r_WPRoles] ON [r_WPRoles]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_r_WPRoles', N'Last', N'UPDATE'
GO
