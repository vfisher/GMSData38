CREATE TABLE [dbo].[r_EmpFiles] (
  ...
  CONSTRAINT [pk_r_EmpFiles] PRIMARY KEY CLUSTERED ([EmpID], [SrcPosID])
)
ON [PRIMARY]
GO

CREATE INDEX [EmpDocID]
  ON [dbo].[r_EmpFiles] ([EmpDocID])
  ON [PRIMARY]
GO

CREATE INDEX [EmpID]
  ON [dbo].[r_EmpFiles] ([EmpID])
  ON [PRIMARY]
GO

CREATE INDEX [FileDate]
  ON [dbo].[r_EmpFiles] ([FileDate])
  ON [PRIMARY]
GO

CREATE INDEX [FileID]
  ON [dbo].[r_EmpFiles] ([FileID])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_r_EmpFiles] ON [r_EmpFiles]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_r_EmpFiles', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_r_EmpFiles] ON [r_EmpFiles]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_r_EmpFiles', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_r_EmpFiles] ON [r_EmpFiles]
FOR DELETE AS
...
EXEC sp_settriggerorder N'dbo.TRel3_Del_r_EmpFiles', N'Last', N'DELETE'
GO