CREATE TABLE [dbo].[r_ProdMP] (
  ...
  CONSTRAINT [_pk_r_ProdMP] PRIMARY KEY CLUSTERED ([ProdID], [PLID])
)
ON [PRIMARY]
GO

CREATE INDEX [CurrID]
  ON [dbo].[r_ProdMP] ([CurrID])
  ON [PRIMARY]
GO

CREATE INDEX [PLID]
  ON [dbo].[r_ProdMP] ([PLID])
  ON [PRIMARY]
GO

CREATE INDEX [PriceMC]
  ON [dbo].[r_ProdMP] ([PriceMC])
  ON [PRIMARY]
GO

CREATE INDEX [ProdID]
  ON [dbo].[r_ProdMP] ([ProdID])
  ON [PRIMARY]
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.r_ProdMP.ProdID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.r_ProdMP.PLID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.r_ProdMP.PriceMC'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.r_ProdMP.CurrID'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_r_ProdMP] ON [r_ProdMP]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_r_ProdMP', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_r_ProdMP] ON [r_ProdMP]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_r_ProdMP', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_r_ProdMP] ON [r_ProdMP]
FOR DELETE AS
...
EXEC sp_settriggerorder N'dbo.TRel3_Del_r_ProdMP', N'Last', N'DELETE'
GO