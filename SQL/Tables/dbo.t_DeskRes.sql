CREATE TABLE [dbo].[t_DeskRes] (
  ...
  CONSTRAINT [pk_t_DeskRes] PRIMARY KEY CLUSTERED ([ChID])
)
ON [PRIMARY]
GO

CREATE INDEX [DocDate]
  ON [dbo].[t_DeskRes] ([DocDate])
  ON [PRIMARY]
GO

CREATE INDEX [DocID]
  ON [dbo].[t_DeskRes] ([DocID])
  ON [PRIMARY]
GO

CREATE INDEX [EmpID]
  ON [dbo].[t_DeskRes] ([EmpID])
  ON [PRIMARY]
GO

CREATE INDEX [IntDocID]
  ON [dbo].[t_DeskRes] ([IntDocID])
  ON [PRIMARY]
GO

CREATE INDEX [OurID]
  ON [dbo].[t_DeskRes] ([OurID])
  ON [PRIMARY]
GO

CREATE INDEX [StockID]
  ON [dbo].[t_DeskRes] ([StockID])
  ON [PRIMARY]
GO

CREATE UNIQUE INDEX [UniqueIndex]
  ON [dbo].[t_DeskRes] ([OurID], [DocID])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_t_DeskRes] ON [t_DeskRes]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_t_DeskRes', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_t_DeskRes] ON [t_DeskRes]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_t_DeskRes', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_t_DeskRes] ON [t_DeskRes]
FOR DELETE AS
...
EXEC sp_settriggerorder N'dbo.TRel3_Del_t_DeskRes', N'Last', N'DELETE'
GO