CREATE TABLE [dbo].[t_LogDiscExpP] (
  ...
  CONSTRAINT [pk_t_LogDiscExpP] PRIMARY KEY CLUSTERED ([LogID])
)
ON [PRIMARY]
GO

CREATE INDEX [DiscCode]
  ON [dbo].[t_LogDiscExpP] ([DiscCode])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_t_LogDiscExpP] ON [t_LogDiscExpP]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_t_LogDiscExpP', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_t_LogDiscExpP] ON [t_LogDiscExpP]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_t_LogDiscExpP', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_t_LogDiscExpP] ON [t_LogDiscExpP]
FOR DELETE AS
...
EXEC sp_settriggerorder N'dbo.TRel3_Del_t_LogDiscExpP', N'Last', N'DELETE'
GO

ALTER TABLE [dbo].[t_LogDiscExpP]
  ADD CONSTRAINT [FK_t_LogDiscExpP_r_DCards] FOREIGN KEY ([DCardChID]) REFERENCES [dbo].[r_DCards] ([ChID]) ON UPDATE CASCADE
GO

ALTER TABLE [dbo].[t_LogDiscExpP]
  ADD CONSTRAINT [FK_t_LogDiscExpP_r_Discs] FOREIGN KEY ([DiscCode]) REFERENCES [dbo].[r_Discs] ([DiscCode]) ON UPDATE CASCADE
GO