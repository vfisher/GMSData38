CREATE TABLE [dbo].[v_Tables] (
  ...
  CONSTRAINT [_pk_v_Tables] PRIMARY KEY CLUSTERED ([RepID], [TableIdx])
)
ON [PRIMARY]
GO

CREATE INDEX [TableCode]
  ON [dbo].[v_Tables] ([TableCode])
  ON [PRIMARY]
GO

CREATE INDEX [TableIdx]
  ON [dbo].[v_Tables] ([TableIdx])
  ON [PRIMARY]
GO

CREATE UNIQUE INDEX [UniqueIndex]
  ON [dbo].[v_Tables] ([RepID], [SourceID], [RelName])
  ON [PRIMARY]
GO

CREATE INDEX [v_Sourcesv_Tables]
  ON [dbo].[v_Tables] ([RepID], [SourceID])
  ON [PRIMARY]
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.v_Tables.RepID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.v_Tables.SourceID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.v_Tables.TableIdx'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.v_Tables.TableCode'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.v_Tables.JoinLevel'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.v_Tables.JoinType'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_v_Tables] ON [v_Tables]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_v_Tables', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_v_Tables] ON [v_Tables]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_v_Tables', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_v_Tables] ON [v_Tables]FOR DELETE AS/* v_Tables - Анализатор - Таблицы - DELETE TRIGGER */BEGIN  SET NOCOUNT ON/* v_Tables ^ v_Valids - Удаление в CHILD *//* Анализатор - Таблицы ^ Анализатор - Доступные значения - Удаление в CHILD */  DELETE v_Valids FROM v_Valids a, deleted d WHERE a.RepID = d.RepID AND a.TableIdx = d.TableIdx  IF @@ERROR > 0 RETURNEND
GO

EXEC sp_settriggerorder N'dbo.TRel3_Del_v_Tables', N'Last', N'DELETE'
GO

ALTER TABLE [dbo].[v_Tables]
  ADD CONSTRAINT [FK_v_Tables_z_Relations] FOREIGN KEY ([RelName]) REFERENCES [dbo].[z_Relations] ([RelName]) ON UPDATE CASCADE
GO