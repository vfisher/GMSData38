...
FOR DELETE AS
/* r_ProdC - Справочник товаров: категории - DELETE TRIGGER */
BEGIN
  SET NOCOUNT ON

/* r_ProdC ^ r_Prods - Проверка в CHILD */
/* Справочник товаров: категории ^ Справочник товаров - Проверка в CHILD */
...
/* Справочник товаров: категории ^ Доступные значения - Справочник товаров: 1 группа - Удаление в CHILD */
...
FOR UPDATE AS
/* r_ProdC - Справочник товаров: категории - UPDATE TRIGGER */
BEGIN
  DECLARE @RCount Int
...
/* Справочник товаров: категории ^ Справочник товаров - Обновление CHILD */
  IF UPDATE(PCatID)
    BEGIN
      IF @RCount = 1
        BEGIN
      ...
      ELSE IF EXISTS (SELECT * FROM r_Prods a, deleted d WHERE a.PCatID = d.PCatID)
        BEGIN
          RAISERROR ('Каскадная операция невозможна ''Справочник товаров: категории'' => ''Справочник товаров''.'
, 18, 1)
          ROLLBACK TRAN
          RETURN
        END
...
/* Справочник товаров: категории ^ Доступные значения - Справочник товаров: 1 группа - Обновление CHILD */
  IF UPDATE(PCatID)
    BEGIN
      IF @RCount = 1
        BEGIN
      ...
      ELSE IF EXISTS (SELECT * FROM z_UserProdC a, deleted d WHERE a.PCatID = d.PCatID)
        BEGIN
          RAISERROR ('Каскадная операция невозможна ''Справочник товаров: категории'' => ''Доступные значения - Справочник товаров: 1 группа''.'
, 18, 1)
          ROLLBACK TRAN
          RETURN
        END
...
FOR INSERT AS
/* r_ProdC - Справочник товаров: категории - INSERT TRIGGER */
BEGIN
  DECLARE @RCount Int
  SELECT @RCount = @@RowCount
  IF @RCount = 0 RETURN
