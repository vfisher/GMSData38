CREATE TABLE [dbo].[z_UserCompG] (
  ...
  CONSTRAINT [pk_z_UserCompG] PRIMARY KEY CLUSTERED ([UserID], [CGrID])
)
ON [PRIMARY]
GO

CREATE INDEX [AccDelete]
  ON [dbo].[z_UserCompG] ([AccDelete])
  ON [PRIMARY]
GO

CREATE INDEX [AccInsert]
  ON [dbo].[z_UserCompG] ([AccInsert])
  ON [PRIMARY]
GO

CREATE INDEX [AccRead]
  ON [dbo].[z_UserCompG] ([AccRead])
  ON [PRIMARY]
GO

CREATE INDEX [AccUpdate]
  ON [dbo].[z_UserCompG] ([AccUpdate])
  ON [PRIMARY]
GO

CREATE INDEX [CGrID]
  ON [dbo].[z_UserCompG] ([CGrID])
  ON [PRIMARY]
GO

CREATE INDEX [UserID]
  ON [dbo].[z_UserCompG] ([UserID])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_z_UserCompG] ON [z_UserCompG]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_z_UserCompG', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_z_UserCompG] ON [z_UserCompG]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_z_UserCompG', N'Last', N'UPDATE'
GO