CREATE TABLE [dbo].[t_EOExpRoutes] (
  ...
  CONSTRAINT [pk_t_EOExpRoutes] PRIMARY KEY CLUSTERED ([ChID], [RouteID])
)
ON [PRIMARY]
GO

CREATE INDEX [RouteID]
  ON [dbo].[t_EOExpRoutes] ([RouteID])
  ON [PRIMARY]
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_t_EOExpRoutes] ON [t_EOExpRoutes]
FOR DELETE AS
...
/* Проверка открытого периода */
  ...
  IF (@ADate IS NOT NULL) 
    BEGIN
      SELECT @Err = FORMATMESSAGE('%s (%s):' + CHAR(13) + dbo.zf_Translate('Дата или одна из дат изменяемого документа меньше даты открытого периода %s для фирмы с кодом %s') ,dbo.zf_Translate('Заказ внешний: Формирование: Маршрут'), 't_EOExpRoutes', dbo.zf_DatetoStr(@ADate), CAST(@OurID as varchar(10)))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
  ...
  IF (@ADate IS NOT NULL) 
    BEGIN
      SELECT @Err = FORMATMESSAGE('%s (%s):' + CHAR(13) + dbo.zf_Translate('Дата или одна из дат изменяемого документа больше даты открытого периода %s для фирмы с кодом %s') ,dbo.zf_Translate('Заказ внешний: Формирование: Маршрут'), 't_EOExpRoutes', dbo.zf_DatetoStr(@ADate), CAST(@OurID as varchar(10)))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
...
EXEC sp_settriggerorder N'dbo.TRel3_Del_t_EOExpRoutes', N'Last', N'DELETE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_t_EOExpRoutes] ON [t_EOExpRoutes]
FOR UPDATE AS
...
/* Проверка открытого периода */
  ...
  IF (@ADate IS NOT NULL) 
    BEGIN
      SELECT @Err = FORMATMESSAGE('%s (%s):' + CHAR(13) + dbo.zf_Translate('Новая дата или одна из дат документа меньше даты открытого периода %s для фирмы с кодом %s') ,dbo.zf_Translate('Заказ внешний: Формирование: Маршрут'), 't_EOExpRoutes', dbo.zf_DatetoStr(@ADate), CAST(@OurID as varchar(10)))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
  ...
  IF (@ADate IS NOT NULL) 
    BEGIN
      SELECT @Err = FORMATMESSAGE('%s (%s):' + CHAR(13) + dbo.zf_Translate('Новая дата или одна из дат документа больше даты открытого периода %s для фирмы с кодом %s') ,dbo.zf_Translate('Заказ внешний: Формирование: Маршрут'), 't_EOExpRoutes', dbo.zf_DatetoStr(@ADate), CAST(@OurID as varchar(10)))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_t_EOExpRoutes', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_t_EOExpRoutes] ON [t_EOExpRoutes]
FOR INSERT AS
...
EXEC sp_settriggerorder N'dbo.TRel1_Ins_t_EOExpRoutes', N'Last', N'INSERT'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TAU3_DEL_t_EOExpRoutes] ON [t_EOExpRoutes]
FOR DELETE
AS
BEGIN
...
SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TAU2_UPD_t_EOExpRoutes] ON [t_EOExpRoutes]
FOR UPDATE
AS
BEGIN
  IF @@RowCount = 0 RETURN
...
SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TAU1_INS_t_EOExpRoutes] ON [t_EOExpRoutes]
FOR INSERT
AS
BEGIN
  IF @@RowCount = 0 RETURN
...
ALTER TABLE [dbo].[t_EOExpRoutes]
  ADD CONSTRAINT [FK_t_EOExpRoutes_t_EOExp] FOREIGN KEY ([ChID]) REFERENCES [dbo].[t_EOExp] ([ChID]) ON DELETE CASCADE ON UPDATE CASCADE
GO