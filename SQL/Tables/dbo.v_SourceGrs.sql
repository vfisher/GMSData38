CREATE TABLE [dbo].[v_SourceGrs] (
  ...
  [ObjectDef] [text] NULL,
  CONSTRAINT [_pk_v_SourceGrs] PRIMARY KEY CLUSTERED ([RepID], [SourceGrName])
)
ON [PRIMARY]
...
CREATE INDEX [PosID]
  ON [dbo].[v_SourceGrs] ([SrcPosID])
  ON [PRIMARY]
GO

CREATE INDEX [RepID]
  ON [dbo].[v_SourceGrs] ([RepID])
  ON [PRIMARY]
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.v_SourceGrs.RepID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.v_SourceGrs.SrcPosID'
GO

EXEC sp_bindefault @defname = N'dbo.DF_Zero', @objname = N'dbo.v_SourceGrs.SourceGrType'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_v_SourceGrs] ON [v_SourceGrs]
FOR UPDATE AS
...
EXEC sp_settriggerorder N'dbo.TRel2_Upd_v_SourceGrs', N'Last', N'UPDATE'
GO

SET QUOTED_IDENTIFIER, ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_v_SourceGrs] ON [v_SourceGrs]FOR DELETE AS/* v_SourceGrs - Анализатор - Группы источников - DELETE TRIGGER */BEGIN  SET NOCOUNT ON/* v_SourceGrs ^ v_MapSG - Удаление в CHILD *//* Анализатор - Группы источников ^ Анализатор - Источники групп - Удаление в CHILD */  DELETE v_MapSG FROM v_MapSG a, deleted d WHERE a.RepID = d.RepID AND a.SourceGrName = d.SourceGrName  IF @@ERROR > 0 RETURNEND
GO

EXEC sp_settriggerorder N'dbo.TRel3_Del_v_SourceGrs', N'Last', N'DELETE'
GO

ALTER TABLE [dbo].[v_SourceGrs]
  ADD CONSTRAINT [FK_v_SourceGrs_v_Reps] FOREIGN KEY ([RepID]) REFERENCES [dbo].[v_Reps] ([RepID]) ON DELETE CASCADE ON UPDATE CASCADE
GO