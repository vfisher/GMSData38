...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dbo].[zf_b_CalcRem]()
...
/* Авансовый отчет (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_RepADP WITH (NOLOCK), b_RepA WITH(NOLOCK)
     WHERE b_RepADP.ProdID = r_Prods.ProdID AND b_RepA.ChID = b_RepADP.ChID AND (r_Prods.InRems <> 0)
...
/* Авансовый отчет валютный (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_ARepADP WITH (NOLOCK), b_ARepA WITH(NOLOCK)
     WHERE b_ARepADP.ProdID = r_Prods.ProdID AND b_ARepA.ChID = b_ARepADP.ChID AND (r_Prods.InRems <> 0)
...
/* Авансовый отчет с признаками (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_CRepADP WITH (NOLOCK), b_CRepA WITH(NOLOCK)
     WHERE b_CRepADP.ProdID = r_Prods.ProdID AND b_CRepA.ChID = b_CRepADP.ChID AND (r_Prods.InRems <> 0)
...
/* ТМЦ: Внутренний расход (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_ExpD WITH (NOLOCK), b_Exp WITH(NOLOCK)
     WHERE b_ExpD.ProdID = r_Prods.ProdID AND b_Exp.ChID = b_ExpD.ChID AND (r_Prods.InRems <> 0)
...
/* ТМЦ: Возврат от получателя (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_RetD WITH (NOLOCK), b_Ret WITH(NOLOCK)
     WHERE b_RetD.ProdID = r_Prods.ProdID AND b_Ret.ChID = b_RetD.ChID AND (r_Prods.InRems <> 0)
...
/* ТМЦ: Возврат поставщику (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_CRetD WITH (NOLOCK), b_CRet WITH(NOLOCK)
     WHERE b_CRetD.ProdID = r_Prods.ProdID AND b_CRet.ChID = b_CRetD.ChID AND (r_Prods.InRems <> 0)
...
/* ТМЦ: Инвентаризация (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), t_PInP WITH (NOLOCK), b_PVenA WITH (NOLOCK), b_PVenD WITH (NOLOCK), b_PVen WITH(NOLOCK)
     WHERE t_PInP.ProdID = r_Prods.ProdID AND b_PVenD.DetProdID = t_PInP.ProdID AND b_PVenD.PPID = t_PInP.PPID AND b_PVen.ChID = b_PVenA.ChID AND b_PVenA.ChID = b_PVenD.ChID AND b_PVenA.ProdID = b_PVenD.DetProdID AND (r_Prods.InRems <> 0)
...
/* ТМЦ: Перемещение (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_PExcD WITH (NOLOCK), b_PExc WITH(NOLOCK)
     WHERE b_PExcD.ProdID = r_Prods.ProdID AND b_PExc.ChID = b_PExcD.ChID AND (r_Prods.InRems <> 0)
...
/* ТМЦ: Переоценка партий (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_PEstD WITH (NOLOCK), b_PEst WITH(NOLOCK)
     WHERE b_PEstD.ProdID = r_Prods.ProdID AND b_PEst.ChID = b_PEstD.ChID AND (r_Prods.InRems <> 0)
...
/* ТМЦ: Приход по ГТД (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_CstD WITH (NOLOCK), b_Cst WITH(NOLOCK)
     WHERE b_CstD.ProdID = r_Prods.ProdID AND b_Cst.ChID = b_CstD.ChID AND (r_Prods.InRems <> 0)
...
/* ТМЦ: Приход по накладной (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_RecD WITH (NOLOCK), b_Rec WITH(NOLOCK)
     WHERE b_RecD.ProdID = r_Prods.ProdID AND b_Rec.ChID = b_RecD.ChID AND (r_Prods.InRems <> 0)
...
/* ТМЦ: Расход по ГТД (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_CInvD WITH (NOLOCK), b_CInv WITH(NOLOCK)
     WHERE b_CInvD.ProdID = r_Prods.ProdID AND b_CInv.ChID = b_CInvD.ChID AND (r_Prods.InRems <> 0)
...
/* ТМЦ: Расходная накладная (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_InvD WITH (NOLOCK), b_Inv WITH(NOLOCK)
     WHERE b_InvD.ProdID = r_Prods.ProdID AND b_Inv.ChID = b_InvD.ChID AND (r_Prods.InRems <> 0)
...
/* ТМЦ: Формирование себестоимости (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_PCostD WITH (NOLOCK), b_PCost WITH(NOLOCK)
     WHERE b_PCostD.ProdID = r_Prods.ProdID AND b_PCost.ChID = b_PCostD.ChID AND (r_Prods.InRems <> 0)
...
/* ТМЦ: Формирование себестоимости (Заголовок) */
    ...
    FROM r_Prods WITH (NOLOCK), b_PCostD WITH (NOLOCK), b_PCost WITH(NOLOCK)
     WHERE b_PCostD.ProdID = r_Prods.ProdID AND b_PCost.ChID = b_PCostD.ChID AND (r_Prods.InRems <> 0) AND (b_PCostD.PPID <> 0)
...
/* Основные средства: Ремонт (ТМЦ) b_SRepDP */
    ...
    FROM r_Prods WITH (NOLOCK), b_SRepDP WITH (NOLOCK), b_SRep WITH(NOLOCK)
     WHERE b_SRepDP.ProdID = r_Prods.ProdID AND b_SRep.ChID = b_SRepDP.ChID AND (r_Prods.InRems <> 0)
...
/* Путевой лист (ТМЦ) */
    ...
    FROM r_Prods WITH (NOLOCK), b_WBillA WITH (NOLOCK), b_WBill WITH(NOLOCK)
     WHERE b_WBillA.ProdID = r_Prods.ProdID AND b_WBill.ChID = b_WBillA.ChID AND (r_Prods.InRems <> 0)
