...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dbo].[zf_ReplicaGetField](@Fields varchar(MAX), @Values varchar(MAX), @Delim varchar(200), @FieldName varchar(200))
RETURNS VARCHAR(MAX)
  ...
  WHILE @CharIndex > 0
    BEGIN
      ...
      IF UPPER(@set_field) = UPPER(@FieldName)
          RETURN SUBSTRING(@Values, 1, @CharIndex - 1)      
