...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TAU1_INS_t_SaleD] ON [dbo].[t_SaleD]
FOR INSERT
AS
BEGIN
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TAU2_UPD_t_SaleD] ON [dbo].[t_SaleD]
FOR UPDATE
AS
BEGIN
  IF @@RowCount = 0 RETURN
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TAU3_DEL_t_SaleD] ON [dbo].[t_SaleD]
FOR DELETE
AS
BEGIN
  IF @@RowCount = 0 RETURN
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TGMSRel2_Upd_t_SaleD] ON [dbo].[t_SaleD]
FOR UPDATE AS
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TGMSRel3_Del_t_SaleD] ON [dbo].[t_SaleD]
FOR DELETE AS
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_t_SaleD] ON [dbo].[t_SaleD]
FOR INSERT AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_t_SaleD]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_t_SaleD] ON [dbo].[t_SaleD]
FOR UPDATE AS
/* t_SaleD - Продажа товара оператором: Продажи товара - UPDATE TRIGGER */
BEGIN
  DECLARE @RCount Int
...
End
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_t_SaleD]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_t_SaleD] ON [dbo].[t_SaleD]
FOR DELETE AS
/* t_SaleD - Продажа товара оператором: Продажи товара - DELETE TRIGGER */
BEGIN
  SET NOCOUNT ON
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_t_SaleD]', 'last', 'delete', null
GO
