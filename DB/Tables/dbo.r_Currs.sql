...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_r_Currs] ON [dbo].[r_Currs]
FOR INSERT AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_r_Currs]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_r_Currs] ON [dbo].[r_Currs]
FOR UPDATE AS
...
/* r_Currs ^ t_DeskRes - Обновление CHILD */
/* Справочник валют ^ Ресторан: Резервирование столиков - Обновление CHILD */
  IF UPDATE(CurrID)
    BEGIN
      IF @RCount = 1
        BEGIN
          UPDATE a SET a.CurrID = i.CurrID
          FROM t_DeskRes a, inserted i, deleted d WHERE a.CurrID = d.CurrID
          IF @@ERROR > 0 RETURN
        END
      ELSE IF EXISTS (SELECT * FROM t_DeskRes a, deleted d WHERE a.CurrID = d.CurrID)
        BEGIN
          RAISERROR ('Каскадная операция невозможна ''Справочник валют'' => ''Ресторан: Резервирование столиков''.'
, 18, 1)
          ROLLBACK TRAN
          RETURN
        END
...
End
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_r_Currs]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_r_Currs] ON [dbo].[r_Currs]
FOR DELETE AS
...
/* r_Currs ^ t_DeskRes - Проверка в CHILD */
/* Справочник валют ^ Ресторан: Резервирование столиков - Проверка в CHILD */
  IF EXISTS (SELECT * FROM t_DeskRes a WITH(NOLOCK), deleted d WHERE a.CurrID = d.CurrID)
    BEGIN
      EXEC z_RelationError 'r_Currs', 't_DeskRes', 3
      RETURN
    END
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_r_Currs]', 'last', 'delete', null
GO
