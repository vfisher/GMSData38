...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_t_SpecD] ON [dbo].[t_SpecD]
FOR INSERT AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_t_SpecD]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_t_SpecD] ON [dbo].[t_SpecD]
FOR UPDATE AS
/* t_SpecD - Калькуляционная карта: Состав - UPDATE TRIGGER */
BEGIN
  DECLARE @RCount Int
  SELECT @RCount = @@RowCount
  IF @RCount = 0 RETURN
...
/* Калькуляционная карта: Состав ^ Справочник товаров - Проверка в PARENT */
  IF UPDATE(ProdID)
    IF EXISTS (SELECT * FROM inserted i WHERE i.ProdID NOT IN (SELECT ProdID FROM r_Prods))
      BEGIN
        EXEC z_RelationError 'r_Prods', 't_SpecD', 1
        RETURN
      END
...
/* Калькуляционная карта: Состав ^ Калькуляционная карта: Заголовок - Проверка в PARENT */
  IF UPDATE(ChID)
    IF EXISTS (SELECT * FROM inserted i WHERE i.ChID NOT IN (SELECT ChID FROM t_Spec))
      BEGIN
        EXEC z_RelationError 't_Spec', 't_SpecD', 1
        RETURN
      END

END
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_t_SpecD]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_t_SpecD] ON [dbo].[t_SpecD]
FOR DELETE AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_t_SpecD]', 'last', 'delete', null
GO
