...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_p_ESicD] ON [dbo].[p_ESicD]
FOR INSERT AS
...
/* Проверка открытого периода */
  ...
  IF @ADate IS NOT NULL
    BEGIN
      SELECT @Err = 'Больничный лист (Данные периодов) (p_ESicD):' + CHAR(13) + 'Новая дата или одна из дат документа меньше даты открытого периода ' + dbo.zf_DatetoStr(@ADate) + ' для фирмы с кодом ' + CAST(@OurID AS varchar(10))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
  ...
  IF @ADate IS NOT NULL
    BEGIN
      SELECT @Err = 'Больничный лист (Данные периодов) (p_ESicD):' + CHAR(13) + 'Новая дата или одна из дат документа больше даты открытого периода ' + dbo.zf_DatetoStr(@ADate) + ' для фирмы с кодом ' + CAST(@OurID as varchar(10))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_p_ESicD]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_p_ESicD] ON [dbo].[p_ESicD]
FOR UPDATE AS
...
/* Проверка открытого периода */
  ...
  IF (@ADate IS NOT NULL) 
    BEGIN
      SELECT @Err = 'Больничный лист (Данные периодов) (p_ESicD):' + CHAR(13) + 'Дата или одна из дат изменяемого документа меньше даты открытого периода ' + dbo.zf_DatetoStr(@ADate) + ' для фирмы с кодом ' + CAST(@OurID as varchar(10))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
  ...
  IF (@ADate IS NOT NULL) 
    BEGIN
      SELECT @Err = 'Больничный лист (Данные периодов) (p_ESicD):' + CHAR(13) + 'Дата или одна из дат изменяемого документа больше даты открытого периода ' + dbo.zf_DatetoStr(@ADate) + ' для фирмы с кодом ' + CAST(@OurID as varchar(10))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
...
End
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_p_ESicD]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_p_ESicD] ON [dbo].[p_ESicD]
FOR DELETE AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_p_ESicD]', 'last', 'delete', null
GO
