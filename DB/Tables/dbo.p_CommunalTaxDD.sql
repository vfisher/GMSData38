...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_p_CommunalTaxDD] ON [dbo].[p_CommunalTaxDD]
FOR INSERT AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_p_CommunalTaxDD]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_p_CommunalTaxDD] ON [dbo].[p_CommunalTaxDD]
FOR UPDATE AS
...
End
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_p_CommunalTaxDD]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_p_CommunalTaxDD] ON [dbo].[p_CommunalTaxDD]FOR DELETE AS/* p_CommunalTaxDD - Коммунальный налог: Подробно - DELETE TRIGGER */BEGIN  SET NOCOUNT ON/* Удаление проводок */  DELETE FROM b_GTran WHERE GTranID IN (SELECT GTranID FROM deleted)/* Удаление регистрации создания записи */  DELETE z_LogCreate FROM z_LogCreate m, deleted i  WHERE m.TableCode = 15070003 AND m.PKValue =     '[' + cast(i.AChID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.CostGAccID as varchar(200)) + ']'/* Удаление регистрации изменения записи */  DELETE z_LogUpdate FROM z_LogUpdate m, deleted i  WHERE m.TableCode = 15070003 AND m.PKValue =     '[' + cast(i.AChID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.CostGAccID as varchar(200)) + ']'/* Регистрация удаления записи */  INSERT INTO z_LogDelete (TableCode, ChID, PKValue, UserCode)  SELECT 15070003, 0,     '[' + cast(d.AChID as varchar(200)) + ']' + ' \ ' +     '[' + cast(d.CostGAccID as varchar(200)) + ']'          , dbo.zf_GetUserCode() FROM deleted dEND
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_p_CommunalTaxDD]', 'last', 'delete', null
GO
