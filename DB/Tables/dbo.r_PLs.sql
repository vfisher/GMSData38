...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_r_PLs] ON [dbo].[r_PLs]
FOR INSERT AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_r_PLs]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_r_PLs] ON [dbo].[r_PLs]
FOR UPDATE AS
...
/* r_PLs ^ t_DeskResD - Обновление CHILD */
/* Справочник прайс-листов ^ Ресторан: Резервирование столиков: Товар - Обновление CHILD */
  IF UPDATE(PLID)
    BEGIN
      IF @RCount = 1
        BEGIN
          UPDATE a SET a.PLID = i.PLID
          FROM t_DeskResD a, inserted i, deleted d WHERE a.PLID = d.PLID
          IF @@ERROR > 0 RETURN
        END
      ELSE IF EXISTS (SELECT * FROM t_DeskResD a, deleted d WHERE a.PLID = d.PLID)
        BEGIN
          RAISERROR ('Каскадная операция невозможна ''Справочник прайс-листов'' => ''Ресторан: Резервирование столиков: Товар''.'
, 18, 1)
          ROLLBACK TRAN
          RETURN
        END
...
End
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_r_PLs]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_r_PLs] ON [dbo].[r_PLs]
FOR DELETE AS
...
/* r_PLs ^ t_DeskResD - Проверка в CHILD */
/* Справочник прайс-листов ^ Ресторан: Резервирование столиков: Товар - Проверка в CHILD */
  IF EXISTS (SELECT * FROM t_DeskResD a WITH(NOLOCK), deleted d WHERE a.PLID = d.PLID)
    BEGIN
      EXEC z_RelationError 'r_PLs', 't_DeskResD', 3
      RETURN
    END
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_r_PLs]', 'last', 'delete', null
GO
