...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TAU2_UPD_b_PVen] ON [dbo].[b_PVen]
FOR UPDATE
AS
BEGIN
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TAU3_DEL_b_PVen] ON [dbo].[b_PVen]
FOR DELETE
AS
BEGIN
  IF @@RowCount = 0 RETURN
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_b_PVen] ON [dbo].[b_PVen]
FOR INSERT AS
...
/* Проверка открытого периода */
  ...
  IF @ADate IS NOT NULL
    BEGIN
      SELECT @Err = 'ТМЦ: Инвентаризация (Заголовок) (b_PVen):' + CHAR(13) + 'Новая дата или одна из дат документа меньше даты открытого периода ' + dbo.zf_DatetoStr(@ADate) + ' для фирмы с кодом ' + CAST(@OurID AS varchar(10))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
  ...
  IF @ADate IS NOT NULL
    BEGIN
      SELECT @Err = 'ТМЦ: Инвентаризация (Заголовок) (b_PVen):' + CHAR(13) + 'Новая дата или одна из дат документа больше даты открытого периода ' + dbo.zf_DatetoStr(@ADate) + ' для фирмы с кодом ' + CAST(@OurID as varchar(10))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
...
/* Обработка статуса */
  IF EXISTS(SELECT * FROM inserted i WHERE dbo.zf_IsValidDocState(14122, i.StateCode) = 0)
    BEGIN
      RAISERROR ('Документ ''ТМЦ: Инвентаризация'' не может иметь указанный статус.', 18, 1)
      ROLLBACK TRAN
      RETURN
    END
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_b_PVen]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_b_PVen] ON [dbo].[b_PVen]
FOR UPDATE AS
...
/* Проверка открытого периода */
  ...
  IF (@ADate IS NOT NULL) 
    BEGIN
      SELECT @Err = 'ТМЦ: Инвентаризация (Заголовок) (b_PVen):' + CHAR(13) + 'Дата или одна из дат изменяемого документа меньше даты открытого периода ' + dbo.zf_DatetoStr(@ADate) + ' для фирмы с кодом ' + CAST(@OurID as varchar(10))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
  ...
  IF (@ADate IS NOT NULL) 
    BEGIN
      SELECT @Err = 'ТМЦ: Инвентаризация (Заголовок) (b_PVen):' + CHAR(13) + 'Дата или одна из дат изменяемого документа больше даты открытого периода ' + dbo.zf_DatetoStr(@ADate) + ' для фирмы с кодом ' + CAST(@OurID as varchar(10))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
...
/* Обработка статуса */
  IF UPDATE(StateCode) AND EXISTS(SELECT * FROM inserted i, deleted d WHERE i.ChID = d.ChID AND dbo.zf_CanChangeState(14122, i.ChID, d.StateCode, i.StateCode) = 0)
    BEGIN
      RAISERROR ('Переход в указанный статус невозможен (ТМЦ: Инвентаризация).', 18, 1)
      ROLLBACK TRAN
      RETURN
    END
...
End
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_b_PVen]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_b_PVen] ON [dbo].[b_PVen]
FOR DELETE AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_b_PVen]', 'last', 'delete', null
GO
