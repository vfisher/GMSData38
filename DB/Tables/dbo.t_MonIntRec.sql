...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_t_MonIntRec] ON [dbo].[t_MonIntRec]
FOR INSERT AS
...
BEGIN
  ...
  IF EXISTS(SELECT * FROM inserted i WHERE dbo.zf_IsValidDocState(11051, i.StateCode) = 0)
    BEGIN
      RAISERROR ('Документ ''Служебный приход денег'' не может иметь указанный статус.', 18, 1)
      ROLLBACK TRAN
      RETURN
    END
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_t_MonIntRec]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_t_MonIntRec] ON [dbo].[t_MonIntRec]
FOR UPDATE AS
...
BEGIN
  ...
  IF UPDATE(StateCode) AND EXISTS(SELECT * FROM inserted i, deleted d WHERE i.ChID = d.ChID AND dbo.zf_CanChangeState(11051, i.ChID, d.StateCode, i.StateCode) = 0)
    BEGIN
      RAISERROR ('Переход в указанный статус невозможен (Служебный приход денег).', 18, 1)
      ROLLBACK TRAN
      RETURN
    END
...
/* Возможно ли редактирование документа */
  ...
  SELECT @StateCodePosID = colid FROM syscolumns WHERE id = object_id('t_MonIntRec') AND name = 'StateCode'
  DECLARE @BytePos int
  DECLARE @UpdLen int
  ...
  IF @FieldsChanged = 1
    IF EXISTS(SELECT * FROM deleted a WHERE dbo.zf_CanChangeDoc(11051, a.ChID, a.StateCode) = 0)
      BEGIN
        RAISERROR ('Изменение документа ''Служебный приход денег'' в данном статусе запрещено.', 18, 1)
        ROLLBACK TRAN
        RETURN
      END
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_t_MonIntRec]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_t_MonIntRec] ON [dbo].[t_MonIntRec]
FOR DELETE AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_t_MonIntRec]', 'last', 'delete', null
GO
