...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_r_StateRuleUsers] ON [dbo].[r_StateRuleUsers]
FOR INSERT AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_r_StateRuleUsers]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_r_StateRuleUsers] ON [dbo].[r_StateRuleUsers]
FOR UPDATE AS
...
End
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_r_StateRuleUsers]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_r_StateRuleUsers] ON [dbo].[r_StateRuleUsers]FOR DELETE AS/* r_StateRuleUsers - Справочник статусов: пользователи - DELETE TRIGGER */BEGIN  SET NOCOUNT ON/* Удаление регистрации создания записи */  DELETE z_LogCreate FROM z_LogCreate m, deleted i  WHERE m.TableCode = 10190003 AND m.PKValue =     '[' + cast(i.StateRuleCode as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.UserCode as varchar(200)) + ']'/* Удаление регистрации изменения записи */  DELETE z_LogUpdate FROM z_LogUpdate m, deleted i  WHERE m.TableCode = 10190003 AND m.PKValue =     '[' + cast(i.StateRuleCode as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.UserCode as varchar(200)) + ']'/* Регистрация удаления записи */  INSERT INTO z_LogDelete (TableCode, ChID, PKValue, UserCode)  SELECT 10190003, 0,     '[' + cast(d.StateRuleCode as varchar(200)) + ']' + ' \ ' +     '[' + cast(d.UserCode as varchar(200)) + ']'          , dbo.zf_GetUserCode() FROM deleted dEND
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_r_StateRuleUsers]', 'last', 'delete', null
GO
