...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_r_ProdG] ON [dbo].[r_ProdG]
FOR INSERT AS
/* r_ProdG - Справочник товаров: 2 группа - INSERT TRIGGER */
BEGIN
  DECLARE @RCount Int
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_r_ProdG]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_r_ProdG] ON [dbo].[r_ProdG]
FOR UPDATE AS
/* r_ProdG - Справочник товаров: 2 группа - UPDATE TRIGGER */
BEGIN
  DECLARE @RCount Int
  SELECT @RCount = @@RowCount
  IF @RCount = 0 RETURN
...
/* Справочник товаров: 2 группа ^ Доступные значения - Справочник товаров: 2 группа - Обновление CHILD */
  IF UPDATE(PGrID)
    BEGIN
      IF @RCount = 1
        BEGIN
      ...
      ELSE IF EXISTS (SELECT * FROM z_UserProdG a, deleted d WHERE a.PGrID = d.PGrID)
        BEGIN
          RAISERROR ('Каскадная операция невозможна ''Справочник товаров: 2 группа'' => ''Доступные значения - Справочник товаров: 2 группа''.'
, 18, 1)
          ROLLBACK TRAN
          RETURN
        END
...
End
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_r_ProdG]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_r_ProdG] ON [dbo].[r_ProdG]
FOR DELETE AS
/* r_ProdG - Справочник товаров: 2 группа - DELETE TRIGGER */
BEGIN
  SET NOCOUNT ON
...
/* Справочник товаров: 2 группа ^ Доступные значения - Справочник товаров: 2 группа - Удаление в CHILD */
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_r_ProdG]', 'last', 'delete', null
GO
