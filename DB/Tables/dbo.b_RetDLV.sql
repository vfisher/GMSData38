...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TAU1_INS_b_RetDLV] ON [dbo].[b_RetDLV]
FOR INSERT
AS
BEGIN
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TAU2_UPD_b_RetDLV] ON [dbo].[b_RetDLV]
FOR UPDATE
AS
BEGIN
  IF @@RowCount = 0 RETURN
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TAU3_DEL_b_RetDLV] ON [dbo].[b_RetDLV]
FOR DELETE
AS
BEGIN
  IF @@RowCount = 0 RETURN
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_b_RetDLV] ON [dbo].[b_RetDLV]
FOR INSERT AS
...
/* Проверка открытого периода */
  ...
  IF @ADate IS NOT NULL
    BEGIN
      SELECT @Err = 'ТМЦ: Возврат от получателя (Сборы по товару) (b_RetDLV):' + CHAR(13) + 'Новая дата или одна из дат документа меньше даты открытого периода ' + dbo.zf_DatetoStr(@ADate) + ' для фирмы с кодом ' + CAST(@OurID AS varchar(10))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
  ...
  IF @ADate IS NOT NULL
    BEGIN
      SELECT @Err = 'ТМЦ: Возврат от получателя (Сборы по товару) (b_RetDLV):' + CHAR(13) + 'Новая дата или одна из дат документа больше даты открытого периода ' + dbo.zf_DatetoStr(@ADate) + ' для фирмы с кодом ' + CAST(@OurID as varchar(10))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_b_RetDLV]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_b_RetDLV] ON [dbo].[b_RetDLV]
FOR UPDATE AS
...
/* Проверка открытого периода */
  ...
  IF (@ADate IS NOT NULL) 
    BEGIN
      SELECT @Err = 'ТМЦ: Возврат от получателя (Сборы по товару) (b_RetDLV):' + CHAR(13) + 'Дата или одна из дат изменяемого документа меньше даты открытого периода ' + dbo.zf_DatetoStr(@ADate) + ' для фирмы с кодом ' + CAST(@OurID as varchar(10))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
  ...
  IF (@ADate IS NOT NULL) 
    BEGIN
      SELECT @Err = 'ТМЦ: Возврат от получателя (Сборы по товару) (b_RetDLV):' + CHAR(13) + 'Дата или одна из дат изменяемого документа больше даты открытого периода ' + dbo.zf_DatetoStr(@ADate) + ' для фирмы с кодом ' + CAST(@OurID as varchar(10))
      RAISERROR (@Err, 18, 1)
      ROLLBACK TRAN
      RETURN
    END
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_b_RetDLV]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_b_RetDLV] ON [dbo].[b_RetDLV]
FOR DELETE AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_b_RetDLV]', 'last', 'delete', null
GO
