...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_z_DocShed] ON [dbo].[z_DocShed]
FOR INSERT AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_z_DocShed]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_z_DocShed] ON [dbo].[z_DocShed]
FOR UPDATE AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_z_DocShed]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_z_DocShed] ON [dbo].[z_DocShed]FOR DELETE AS/* z_DocShed - Документы - Процессы - DELETE TRIGGER */BEGIN  SET NOCOUNT ON/* Удаление регистрации изменения статуса */  DELETE z_LogState FROM z_LogState m, deleted i WHERE m.DocCode = 1001 AND m.ChID = i.ChIDEND
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_z_DocShed]', 'last', 'delete', null
GO
