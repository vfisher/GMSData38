...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_b_RemD] ON [dbo].[b_RemD]
FOR INSERT AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_b_RemD]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_b_RemD] ON [dbo].[b_RemD]
FOR UPDATE AS
...
End
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_b_RemD]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_b_RemD] ON [dbo].[b_RemD]FOR DELETE AS/* b_RemD - ТМЦ: Остатки на дату (Данные) - DELETE TRIGGER */BEGIN  SET NOCOUNT ON/* Удаление регистрации создания записи */  DELETE z_LogCreate FROM z_LogCreate m, deleted i  WHERE m.TableCode = 14915001 AND m.PKValue =     '[' + cast(i.OurID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.StockID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.ProdID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.PPID as varchar(200)) + ']'/* Удаление регистрации изменения записи */  DELETE z_LogUpdate FROM z_LogUpdate m, deleted i  WHERE m.TableCode = 14915001 AND m.PKValue =     '[' + cast(i.OurID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.StockID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.ProdID as varchar(200)) + ']' + ' \ ' +     '[' + cast(i.PPID as varchar(200)) + ']'/* Регистрация удаления записи */  INSERT INTO z_LogDelete (TableCode, ChID, PKValue, UserCode)  SELECT 14915001, 0,     '[' + cast(d.OurID as varchar(200)) + ']' + ' \ ' +     '[' + cast(d.StockID as varchar(200)) + ']' + ' \ ' +     '[' + cast(d.ProdID as varchar(200)) + ']' + ' \ ' +     '[' + cast(d.PPID as varchar(200)) + ']'          , dbo.zf_GetUserCode() FROM deleted dEND
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_b_RemD]', 'last', 'delete', null
GO
