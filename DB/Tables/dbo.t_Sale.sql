...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TAU2_UPD_t_Sale] ON [dbo].[t_Sale]
FOR UPDATE
AS
BEGIN
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TAU3_DEL_t_Sale] ON [dbo].[t_Sale]
FOR DELETE
AS
BEGIN
  IF @@RowCount = 0 RETURN
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TGMSRel2_Upd_t_Sale] ON [dbo].[t_Sale]
FOR UPDATE AS
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TGMSRel3_Del_t_Sale] ON [dbo].[t_Sale]
FOR DELETE AS
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel1_Ins_t_Sale] ON [dbo].[t_Sale]
FOR INSERT AS
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel1_Ins_t_Sale]', 'last', 'insert', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel2_Upd_t_Sale] ON [dbo].[t_Sale]
FOR UPDATE AS
/* t_Sale - Продажа товара оператором: Заголовок - UPDATE TRIGGER */
BEGIN
  DECLARE @RCount Int
...
End
GO
EXEC sp_settriggerorder N'[dbo].[TRel2_Upd_t_Sale]', 'last', 'update', null
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[TRel3_Del_t_Sale] ON [dbo].[t_Sale]
FOR DELETE AS
/* t_Sale - Продажа товара оператором: Заголовок - DELETE TRIGGER */
BEGIN
  SET NOCOUNT ON
...
END
GO
EXEC sp_settriggerorder N'[dbo].[TRel3_Del_t_Sale]', 'last', 'delete', null
GO
