...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[t_SaleGetTotals](
  @ChID bigint,
...
AS
BEGIN
  DECLARE @ASum1 numeric(21, 9), @ASum2 numeric(21, 9), @TaxTypeID int, @ChequeDisc numeric(21, 9), @PosDisc numeric(21, 9)

  SELECT @Disc = Discount FROM t_SaleTemp WHERE ChID = @ChID

  DECLARE ChequeCursor CURSOR FOR
  SELECT PurSumCC_wt, /* dbo.tf_SaleGetPosWithDisc(@ChID, SrcPosID, 0) */ SumCC_wt, TaxTypeID, 100 - PriceCC_wt / PurPriceCC_wt * 100 Discount
  FROM t_SaleTempD
  WHERE ChID = @ChID
  ...
  INTO @ASum1, @ASum2, @TaxTypeID, @PosDisc
