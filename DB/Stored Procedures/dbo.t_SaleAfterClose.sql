    ...
    BEGIN
      IF @Qty IS NOT NULL /* Это абонемент. Нужно найти акцию для начисления бонусов */
        BEGIN
          ...
          IF @DiscCode is NULL
            SELECT @Msg = dbo.zf_Translate('Чек закрыт успешно, однако бонусы для абонемента не были начислены корректно, т.к. не найдена акция дисконтной системы! Обратитесь к администратору.')
         END
