...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[t_VenCalculateAll] 
(
    @ChID bigint
)
AS
BEGIN
  DECLARE 
    ...
    @SrcPosID int, @DSum decimal(21,9), @PPQty decimal(21,9), 
  ...
  DECLARE VenACursor CURSOR LOCAL FAST_FORWARD 
  FOR 
  SELECT ProdID, TNewQty, UM
  FROM t_VenA WHERE ChID = @ChID
  ORDER BY TSrcPosID

  OPEN VenACursor
      FETCH NEXT FROM VenACursor
      INTO @ProdID, @TNewQty, @UM
  ...
  BEGIN
    ...
    WHILE @@FETCH_STATUS = 0
      BEGIN
       SELECT
        ...
        SELECT 
            @ChID, @ProdID, @SrcPosID, @PPID, @UM, @PPQty, 
    ...
    IF @TNewQty > @TPQty OR @TZQty > 0
      BEGIN
        ...
        SELECT 
            @ChID, @ProdID, @SrcPosID, @PPID, @UM, - @TZQty, 
